# Specification: OKF Knowledge Base Subcommand Group (`okf-schema kb` / `okfkb`)
...
- The data directory contains an `__init__.py` (empty) so it is a valid package for
  `importlib.resources`.

### 5 — `okfkb install [PATH]`
...
- `importlib.resources.files()` (Python 3.9+) is the required API; no fallback to
  `pkg_resources` or `__file__`-based path manipulation.
