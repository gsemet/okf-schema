...
def read_tier(bundle: Path, tier: str, *, status: str | None = None) -> list[KbNode]:
    ...
    # Dated Finding filenames sort chronologically. Present raw evidence newest
    # first; stable tiers retain their predictable alphabetical order.
    nodes.sort(key=lambda n: n.path, reverse=folder == "findings")
    return nodes
...
def _compare(node: KbNode, cond: _Condition) -> bool:
    ...
    # ``status`` is the query/output alias for KB lifecycle state. Prefer the
    # namespaced OKF 0.2 field while retaining legacy bundle compatibility.
    raw = (
        node.frontmatter.get("kb_status") or node.frontmatter.get("status")
        if key == "status"
        else node.frontmatter.get(key)
    )
